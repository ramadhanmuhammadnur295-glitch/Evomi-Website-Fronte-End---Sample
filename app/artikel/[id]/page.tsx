...
const dict = {
  id: {
    nav: { shop: "Toko", quiz: "Kuis", article: "Artikel", home: "Home", profile: "Profil", orders: "Pesanan", logout: "Keluar", login: "Masuk", register: "Daftar" },
    chat: { open: "Chat Admin", close: "Tutup Chat" },
    status: { loading: "Memuat Jurnal...",notFound: "Artikel Tidak Ditemukan", published: "Diterbitkan pada", author: "Oleh" },
    actions: { back: "Kembali ke Artikel" }
  },
  en: {
    nav: { shop: "Shop", quiz: "Quiz", article: "Articles", home: "Home", profile: "Profile", orders: "Orders", logout: "Logout", login: "Login", register: "Register" },
    chat: { open: "Chat Admin", close: "Close Chat" },
    status: { loading: "Loading Journal...", notFound: "Article Not Found", published: "Published on", author: "By" },
    actions: { back: "Back to Articles" }
  }
};
  ...
  return (
    <div className={`${fontCaption.variable} ${fontJudul.variable} min-h-screen bg-[#FBFBF9] text-stone-900 font-sans antialiased selection:bg-amber-200 selection:text-stone-900`}>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.5, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ delay: 1, duration: 0.8, type: "spring" }} className="fixed bottom-6 right-6 md:bottom-8 md:right-8 z-[90]">
        <button onClick={() => setIsChatOpen(!isChatOpen)} className="relative flex items-center justify-center w-14 h-14 bg-stone-900 rounded-full shadow-[0_10px_40px_rgba(0,0,0,0.15)] hover:shadow-[0_10px_40px_rgba(0,0,0,0.3)] hover:-translate-y-1 transition-all duration-300 group/chatbtn z-0">
          ...
          {!isChatOpen && ( <span className="absolute inset-0 rounded-full bg-stone-500 opacity-20 animate-ping group-hover/chatbtn:animate-none -z-10"></span> )}
          <svg className="w-6 h-6 text-[#FBFBF9] relative z-10 transition-colors duration-300 group-hover/chatbtn:text-stone-950" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      <nav className="fixed w-full z-[100] bg-[#0071bc]/95 backdrop-blur-xl border-b border-white/10 shadow-lg transition-all duration-300">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-8 h-20 flex items-center justify-between">
          ...
          <div className={`hidden md:flex w-1/3 justify-center items-center space-x-10 ${fontJudul.className} text-[13px] tracking-[0.2em] uppercase text-white`}>
            ...
            <Link href="/artikel" className="relative group/nav py-1 text-amber-400">
              {t.nav.article}
              <span className="absolute bottom-0 left-0 w-full h-[2px] bg-amber-500 scale-x-100 origin-left" />
            </Link>
          </div>

          {/* 3. KANAN: USER / AUTH & TOGGLE BAHASA */}
          <div className="flex-1 md:w-1/3 flex justify-end items-center space-x-4">
            
            {/* Language Toggle (Desktop) - md:mr-8 agar renggang ke kiri */}
            <div className="hidden md:flex items-center relative bg-white/10 backdrop-blur-md border border-white/20 p-1 rounded-full overflow-hidden md:mr-8">
              <button onClick={() => setLang("id")} className={`relative z-10 px-3 py-1 text-[10px] font-bold uppercase tracking-widest transition-colors duration-300 ${lang === 'id' ? 'text-stone-900' : 'text-white hover:text-amber-400'}`}>
                ID
              </button>
              <button onClick={() => setLang("en")} className={`relative z-10 px-3 py-1 text-[10px] font-bold uppercase tracking-widest transition-colors duration-300 ${lang === 'en' ? 'text-stone-900' : 'text-white hover:text-amber-400'}`}>
                EN
              </button>
            ...
            <div className="hidden md:flex items-center space-x-6">
              {user ? (
                <div className="relative">
                  <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="group/userbtn relative flex items-center space-x-3 border border-white/30 rounded-full p-1 pr-4 bg-white/10 transition-all duration-300 backdrop-blur-sm overflow-hidden z-0">
            ...
            {/* Mobile Menu Trigger */}
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden p-2 text-white hover:text-amber-400 transition-colors">
            ...
            <motion.div variants={mobileMenuVars} initial="hidden" animate="visible" exit="exit" className="md:hidden bg-[#0071bc] border-t border-white/10 overflow-hidden">
              <div className="px-8 py-10 flex flex-col space-y-8">
                ...
                <motion.div variants={itemVars} className="flex justify-center mb-4">
                  <div className="flex items-center relative bg-white/10 backdrop-blur-md border border-white/20 p-1 rounded-full overflow-hidden w-fit">
                    <button onClick={() => setLang("id")} className={`relative z-10 px-6 py-2 text-xs font-bold uppercase tracking-widest transition-colors duration-300 ${lang === 'id' ? 'text-stone-900' : 'text-white'}`}>ID</button>
                    <button onClick={() => setLang("en")} className={`relative z-10 px-6 py-2 text-xs font-bold uppercase tracking-widest transition-colors duration-300 ${lang === 'en' ? 'text-stone-900' : 'text-white'}`}>EN</button>
                    <motion.div className="absolute top-1 bottom-1 w-[calc(50%-2px)] bg-amber-500 rounded-full shadow-sm z-0" initial={false} animate={{ left: lang === 'id' ? "4px" : "calc(50% + 2px)" }} transition={{ type: "spring", stiffness: 400, damping: 30 }} />
                  </div>
                </motion.div>
                  ...
                  {user ? (
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <Link href="/profile" className="px-4 py-3 bg-white/10 rounded-xl text-[10px] font-bold uppercase text-white text-center">{t.nav.profile}</Link>
                        <Link href="/orders" className="px-4 py-3 bg-white/10 rounded-xl text-[10px] font-bold uppercase text-white text-center">{t.nav.orders}</Link>
                      </div>
                    ...
                    <div className="flex flex-col space-y-4">
                      <Link href="/login" className="w-full py-4 bg-white text-[#0071bc] rounded-xl text-center text-[10px] font-bold uppercase tracking-widest">{t.nav.login}</Link>
                      <Link href="/register" className="w-full py-4 border border-white/30 text-white rounded-xl text-center text-[10px] font-bold uppercase tracking-widest">{t.nav.register}</Link>
                    </div>
                  )}
                </motion.div>
              </div>
      ...
      <main className="pt-32 pb-20 px-6">
        <article className="max-w-4xl mx-auto">
          <motion.div initial="hidden" animate="visible" variants={fadeInUp} className="text-center mb-10">
            ...
            <h1 className={`${fontJudul.className} text-4xl md:text-6xl uppercase mt-8 mb-6 leading-tight text-stone-800`}>
              {article.title}
            </h1>
            <div className="flex flex-col items-center gap-2">
              <p className="text-stone-400 text-[10px] font-bold tracking-widest uppercase">
                {t.status.published}{" "}
                {new Date(article.created_at).toLocaleDateString(lang === "id" ? "id-ID" : "en-US", {
                  day: "2-digit",
                  month: "long",
                  year: "numeric",
                })}
              ...
              <span className="text-[10px] text-amber-900/60 font-bold uppercase tracking-widest">
                {t.status.author} {article.author || "Evomi Editorial"}
              </span>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1 }} className="relative aspect-[21/9] rounded-[2rem] overflow-hidden mb-16 shadow-xl border border-stone-100">
            <Image src={article.image_url || "https://images.unsplash.com/photo-1541643600914-78b084683601?auto=format&fit=crop&q=80&w=1200"} alt={article.title} fill className="object-cover" />
          </motion.div>

          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="max-w-3xl mx-auto">
            <div
              className="article-content prose prose-stone prose-lg max-w-none whitespace-normal break-words prose-headings:font-bold prose-headings:text-stone-900 prose-headings:tracking-tight prose-headings:scroll-mt-32 prose-p:text-stone-700 prose-p:leading-relaxed prose-p:mb-6 prose-p:text-left prose-strong:text-stone-900 prose-strong:font-semibold prose-ul:list-disc prose-ol:list-decimal prose-li:text-stone-700 prose-li:leading-relaxed prose-img:rounded-3xl prose-img:shadow-xl prose-img:my-10 overflow-hidden"
      ...
      <footer className="bg-white pt-20 pb-10 px-6 border-t border-stone-100 mt-20">
        <div className="max-w-7xl mx-auto flex flex-col items-center">
          <Link href="/artikel" className="group flex items-center space-x-4 border border-stone-200 px-10 py-4 rounded-full hover:bg-stone-900 hover:border-stone-900 transition-all duration-500 mb-12">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-stone-400 group-hover:text-white transition-colors duration-500">
