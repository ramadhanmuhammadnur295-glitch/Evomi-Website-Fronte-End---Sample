...
const dict = {
    id: {
        nav: { home: "Home", shop: "Toko", quiz: "Kuis", article: "Artikel", profile: "Profil", orders: "Pesanan", logout: "Keluar", login: "Masuk", register: "Daftar" },
        chat: { open: "Chat Admin", close: "Tutup Chat" },
        hero: { subtitle: "Jurnal Evomi", title1: "ARTIKEL &", title2: "CERITA" },
        loading: "Memuat Cerita...",
        pagination: "Halaman Jurnal",
        footer: {
            desc: "Menghadirkan pengalaman sensorik melalui kurasi aroma terbaik. Dedikasi pada seni artisan fragrance.",
            contact: "Hubungi Kami",
            newsletter: "Buletin",
            newsDesc: "Dapatkan akses eksklusif ke rilis terbaru kami.",
            placeholder: "Alamat email Anda",
            subscribe: "Berlangganan"
        }
    },
    en: {
        nav: { home: "Home", shop: "Shop", quiz: "Quiz", article: "Articles", profile: "Profile", orders: "Orders", logout: "Logout", login: "Login", register: "Register" },
        chat: { open: "Chat Admin", close: "Close Chat" },
        hero: { subtitle: "Evomi Journal", title1: "ARTICLES &", title2: "STORIES" },
        loading: "Loading Stories...",
        pagination: "Journal Page",
        footer: {
            desc: "Delivering sensory experiences through the finest curated aromas. Dedicated to the art of artisan fragrance.",
            contact: "Contact Us",
            newsletter: "The Newsletter",
            newsDesc: "Get exclusive access to our latest releases.",
...
const mobileMenuVars: Variants = {
    ...
    visible: { opacity: 1, height: "auto", transition: { duration: 0.5, ease: [0.21, 0.47, 0.32, 0.98], staggerChildren: 0.1, delayChildren: 0.1 } },
        ...
        <div className={`${fontCaption.variable} ${fontJudul.variable} min-h-screen bg-[#FBFBF9] text-stone-900 font-sans antialiased selection:bg-amber-200 selection:text-stone-900`}>
            ...
            <nav className="fixed w-full z-[100] bg-[#0071bc]/95 backdrop-blur-xl border-b border-white/10 shadow-lg transition-all duration-300">
                ...
                <div className="max-w-7xl mx-auto px-6 md:px-8 h-20 flex items-center justify-between">
                    ...
                    {/* SEKTOR TENGAH: NAV MENU DESKTOP */}
                    ...
                    {/* SEKTOR KANAN: USER PROFILE / LOGIN REGISTER & LANGUAGE TOGGLE */}
                    <div className="flex-1 md:w-1/3 flex justify-end items-center space-x-4">

                        {/* Language Toggle (Desktop) */}
                        {/* Language Toggle (Desktop) - mr-2 diubah ke md:mr-8 agar lebih renggang ke kiri */}
                            ...
                            <div className="px-8 py-10 flex flex-col space-y-8">
                                ...
                                <motion.div variants={itemVars} className="flex justify-center mb-4">
                                    <div className="flex items-center relative bg-white/10 backdrop-blur-md border border-white/20 p-1 rounded-full overflow-hidden w-fit">
                                        <button onClick={() => setLang("id")} className={`relative z-10 px-6 py-2 text-xs font-bold uppercase tracking-widest transition-colors duration-300 ${lang === 'id' ? 'text-stone-900' : 'text-white hover:text-amber-400'}`}>ID</button>
                                        <button onClick={() => setLang("en")} className={`relative z-10 px-6 py-2 text-xs font-bold uppercase tracking-widest transition-colors duration-300 ${lang === 'en' ? 'text-stone-900' : 'text-white hover:text-amber-400'}`}>EN</button>
                                        <motion.div className="absolute top-1 bottom-1 w-[calc(50%-2px)] bg-amber-500 rounded-full shadow-sm z-0" initial={false} animate={{ left: lang === 'id' ? "4px" : "calc(50% + 2px)" }} transition={{ type: "spring", stiffness: 400, damping: 30 }} />
                                    </div>
                                </motion.div>
            ...
            {totalPages > 1 && (
                <div className="flex flex-col items-center mt-20 pb-32 space-y-6">
                    ...
                    <div className="flex items-center space-x-2">
                        <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="p-3 text-stone-400 hover:text-stone-900 disabled:opacity-20 transition-colors">
                        ...
                        <div className="flex space-x-1">
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map((num) => (
                                <button key={num} onClick={() => handlePageChange(num)} className={`w-10 h-10 rounded-full text-[10px] font-bold tracking-widest transition-all duration-300 ${currentPage === num ? "bg-stone-900 text-white shadow-lg" : "bg-transparent text-stone-400 hover:bg-stone-100 hover:text-stone-900"}`}>
                                    {String(num).padStart(2, '0')}
                                </button>
                            ))}
                        </div>

                        <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="p-3 text-stone-400 hover:text-stone-900 disabled:opacity-20 transition-colors">
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <p className="text-[9px] uppercase tracking-[0.3em] text-stone-400 font-bold">
                        {t.pagination} {currentPage} of {totalPages}
                    </p>
                </div>
            )}
            ...
            <motion.footer initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1 }} className="relative z-20 bg-white pt-20 pb-10 px-6 md:px-8 border-t border-stone-100">
                <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-12 mb-16">
                    ...
                    <div className="md:col-span-3">
                        <h4 className="font-bold text-[11px] uppercase tracking-widest mb-6 text-stone-800">{t.footer.contact}</h4>
                    ...
                    <div className="md:col-span-4">
                        <h4 className="font-bold text-[11px] uppercase tracking-widest mb-6 text-stone-800">{t.footer.newsletter}</h4>
                        <p className="text-stone-400 text-xs mb-4">{t.footer.newsDesc}</p>
                        <div className="flex border-b border-stone-300 pb-2 focus-within:border-stone-900 transition-colors">
                            <input type="email" placeholder={t.footer.placeholder} className="bg-transparent w-full text-sm outline-none text-stone-800 placeholder-stone-400" />
                            <button className="text-[10px] uppercase font-bold text-stone-800 hover:text-amber-800 transition-colors tracking-wider">{t.footer.subscribe}</button>
                        </div>
                    </div>
                </div>
